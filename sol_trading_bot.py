...
class SolanaTradingBot:
    ...
    def get_ml_prediction(self, pair, df):
        """Get prediction from ML model with improved error handling"""
        try:
            if pair not in self.ml_models or self.ml_models[pair] is None:
            ...
            if X is None:
                logger.warning(f"Failed to prepare features for {pair}")
                return None, 0.0
