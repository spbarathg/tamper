...
class SolanaTradingBot:
    def __init__(self):
        ...
        # ML model parameters
        self.ml_models = {}  # Dictionary to store models for each pair
    ...
    def calculate_indicators(self, df):
        ...
        df['dmi_plus'] = ta.dmi(df['high'], df['low'], df['close'], length=14)['DMP_14']
        df['dmi_minus'] = ta.dmi(df['high'], df['low'], df['close'], length=14)['DMN_14']
        ...
        # Fibonacci Retracement levels (using recent high and low)
    ...
    def get_ml_prediction(self, pair, df):
        """Get prediction from ML model for a specific pair"""
        if pair not in self.ml_models or self.ml_models[pair] is None or pair not in self.scalers or self.scalers[pair] is None:
            return None, 0.0
        ...
        try:
            ...
            # Scale features
            X_latest_scaled = self.scalers[pair].transform(X_latest)
            
            # Get prediction and probability
            prediction = self.ml_models[pair].predict(X_latest_scaled)[0]
            probabilities = self.ml_models[pair].predict_proba(X_latest_scaled)[0]
            ...
            logger.error(f"Error getting ML prediction for {pair}: {e}")
            return None, 0.0
    ...
    def _calculate_trend_strength(self, df_1h, df_4h, df_1d):
        ...
        # Volume confirmation
        volume_confirmation = 1.0 if df_1h['volume_ratio'].iloc[-1] > self.volume_threshold else 0.5
    ...
    def analyze_signals(self, df_1h, df_4h, df_1d):
        """Analyze trading signals with multiple confirmations and entry window"""
        signals = {
            'long': False,
            'short': False,
            'strength': 0,
            ...
            'entry_window_hours': 1,  # Default 1-hour window
            'reason': ""
        }
        ...
        if final_score >= 0.7:  # High confidence threshold
            ...
            # Calculate entry window (price range to enter within the next hour)
            # Use recent low and high to determine entry window
            recent_low = df_1h['low'].iloc[-5:].min()  # Last 5 candles
            recent_high = df_1h['high'].iloc[-5:].max()  # Last 5 candles
            
            # Set entry window between recent low and current price
            # This gives a reasonable range to place a limit order
            signals['entry_window_low'] = recent_low
            signals['entry_window_high'] = current_price
            
            # Combine reasons for the signal
            ...
            if volatility_factor > 0.03:  # High volatility
                signals['entry_window_hours'] = 0.5  # 30 minutes
            elif volatility_factor > 0.02:  # Medium volatility
                signals['entry_window_hours'] = 1  # 1 hour
            else:  # Low volatility
                signals['entry_window_hours'] = 2  # 2 hours
    ...
    def generate_trading_decision(self, signals):
        ...
        if signals['long']:
            ...
            window_hours = signals['entry_window_hours']
            window_text = f"{int(window_hours)} hour{'s' if window_hours != 1 else ''}"
            if window_hours < 1:
                window_text = f"{int(window_hours * 60)} minutes"
            
            # Create a detailed message
            message = (
                f"SOL/USDT CONVERSION OPPORTUNITY\n"
                f"Action: Convert USDT to SOL\n"
                f"Confidence: {signals['confidence']:.2f}\n"
                f"Entry Window: {window_text}\n"
                f"Entry Price Range: {signals['entry_window_low']:.2f} - {signals['entry_window_high']:.2f} USDT\n"
                f"Stop Loss: {signals['stop_loss']:.2f} USDT\n"
                f"Take Profit: {signals['take_profit']:.2f} USDT\n"
                f"Risk-Reward: 1:2\n"
    ...
    async def run(self):
        ...
        await self.send_telegram_message("🚀 Multi-Pair Trading Bot started! Monitoring multiple cryptocurrency pairs for conversion opportunities.")
        
        # Schedule periodic model retraining (every 7 days)
